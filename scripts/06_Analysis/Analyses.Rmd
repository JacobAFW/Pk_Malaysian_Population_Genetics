...
Might be needed for NRAF plots - moimix 
...
plot_baf <- function(DATA, SAMPLE, FWS){
baf_plot <- DATA %>%
    ...
    ggtitle(paste0("Fws=", FWS))

    ggsave(paste0("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/MAF-Fws_plots/for_pub/", SAMPLE, "_", FWS, ".png"), width = 12, dpi = 300, baf_plot)
}

plot_baf(baf_df, "ERR985395", "0.54")
plot_baf(baf_df, "ERR985405", "0.62") # or ERR985410
plot_baf(baf_df, "ERR2214850", "0.72")
plot_baf(baf_df, "ERR985417", "0.84")
plot_baf(baf_df, "ERR985376", "0.93")
baf_df %>% 
  rename("PK_SB_DNA_004" = "PK_SB_DNA_004_DKDL210002133-1a_HWHGKDSXY_L4") %>%
plot_baf(., "PK_SB_DNA_004", "0.99")
...
# Testing states and clusters
fws_tests <- fws_all %>%
  mutate(State = ifelse(Location == "Clinic" | Location == "Gua_Musang" | Location == "Kuala_Lipis" | Location == "Sungai_Siput" | Location == "Sungai_Siuit" | Location == "Taiping" | Location == "Temerloh", "Peninsular",
  ifelse(Location == "Sabah", "Sabah", "Sarawak"))) 
...
sample_info <- read_tsv("sample_info_updated.tsv") %>%
  mutate(State = ifelse(grepl("Sabah", State), "Sabah", 
  ifelse(State != "Sarawak" & State != "Sabah", "Peninsular", State))) 
...
fws_plot <- fws_all %>% 
  ...
  left_join(sample_info %>% 
    select(-Proportion)) %>%
  filter(!is.na(State)) %>%
  ggplot(aes(y = State, x = Proportion, by = State)) +
  geom_boxplot() +
...
ggsave("moimix/fws_state_boxplot.png", width = 6, height = 8, dpi = 600, fws_plot)
...
```{bash,eval=F}
cut -f 2 Pk.fam | sort | uniq -d > Pk.dups
