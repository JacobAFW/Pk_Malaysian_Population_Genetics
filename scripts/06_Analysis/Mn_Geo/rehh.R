...
iHS <- wgs_ihs$ihs %>%
    ...
    geom_jitter() + 
    geom_hline(yintercept = 5, linetype = 10, colour = "#1F968BFF") +
    geom_hline(yintercept = -5, linetype = 10, colour = "#1F968BFF") +
...
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/iHS.png", dpi = 600, width = 10, iHS)
...
iHS_pvalue <- wgs_ihs$ihs %>%
    ...
    geom_jitter() + 
    geom_hline(yintercept = 10, linetype = 2, colour = "#1F968BFF") + # adjusted with bonferonni
    theme(legend.position = "none") +
    scale_colour_viridis_d() +
    xlab("Chromosomes") +
    ylab("iHS p-value")

ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/iHS_pvalue.png", dpi = 600, width = 10, iHS_pvalue)

## Plotting using rehh

png('/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/iHS_rehh.png')
manhattanplot(wgs_ihs, threshold = c(-5, 5))
dev.off()

png('/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/iHS_p_rehh.png')
manhattanplot(wgs_ihs, pval = TRUE, threshold = 10)
dev.off()


## QQ-Plot
png('/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/iHS_QQ.png')
...
### Sabah
wgscan_Sabah <- tibble("CHR" = NA, "POSITION" = NA, "FREQ_A" = NA, "FREQ_D" = NA, "NHAPLO_A" = NA, "NHAPLO_D" = NA, "IHH_A" = NA, "IHH_D" = NA, "IES" = NA, "INES" = NA)

for(i in chromsomes_files) {
    # haplotype file name for each chromosome
    hap_file = paste(i, "_Sabah.vcf.gz", sep = "") # filename pattern
    # create internal representation
    hh <- data2haplohh(hap_file = hap_file, 
                    chr.name = i, 
                    polarize_vcf = FALSE, # if the AA key is absent
                    ...
                    vcf_reader = "data.table") # use this package
    scan <- scan_hh(hh) # perform scan on a single chromosome (calculate iHH/iES values)
    wgscan_Sabah <- wgscan_Sabah %>% # append the wgscan object every round with additional data
    rbind(scan)
}

### Betong_Kapit_Sarikei
wgscan_Betong_Kapit_Sarikei <- tibble("CHR" = NA, "POSITION" = NA, "FREQ_A" = NA, "FREQ_D" = NA, "NHAPLO_A" = NA, "NHAPLO_D" = NA, "IHH_A" = NA, "IHH_D" = NA, "IES" = NA, "INES" = NA)

for(i in chromsomes_files) {
    # haplotype file name for each chromosome
    hap_file = paste(i, "_Betong-Kapit-Sarikei.vcf.gz", sep = "") # filename pattern
    # create internal representation
    hh <- data2haplohh(hap_file = hap_file, 
                    chr.name = i, 
                    polarize_vcf = FALSE, # if the AA key is absent
                    ...
                    vcf_reader = "data.table") # use this package
    scan <- scan_hh(hh) # perform scan on a single chromosome (calculate iHH/iES values)
    wgscan_Betong_Kapit_Sarikei<- wgscan_Betong_Kapit_Sarikei %>% # append the wgscan object every round with additional data
    rbind(scan)
}

# Rsb
Rsb <- ines2rsb(
        scan_pop1 = wgscan_Sabah,
        scan_pop2 = wgscan_Betong_Kapit_Sarikei,
        popname1 = "Sabah",
...
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/Rsb.png", dpi = 600, width = 10, Rsb_plot)
...
Rsb_pvalue <- Rsb %>%
    ...
    ggplot(aes(x = CHR, y = LOGPVALUE, colour = CHR)) +
    geom_jitter() + 
    geom_hline(yintercept = 5, linetype = 2, colour = "#1F968BFF") + # adjusted with bonferonni
    theme(legend.position = "none") +
    scale_colour_viridis_d() +
    xlab("Chromosomes") +
    ylab("Rsb p-value")

ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/Rsb_pvalue.png", dpi = 600, width = 10, Rsb_pvalue)
...
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/XP_EHH.png", dpi = 600, width = 10, XP_EHH_plot)
...
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/XP_EHH_pvalue.png", dpi = 600, width = 10, XP_EHH_pvalue)
...
Rsb_XP_comaprison <- Rsb %>%
    ...
    na.omit() %>%
    ggplot(aes(x = RSB_Sabah_Betong_Kapit_Sarikei, y = XPEHH_Sabah_Betong_Kapit_Sarikei, colour = CHR)) +
    geom_point() + 
    theme(legend.position = "none") +
    scale_colour_viridis_d() +
    xlab("Rsb") +
...
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/rehh_geo/Mn_plots/XP_Rsb_comparison.png", dpi = 600, width = 10, Rsb_XP_comaprison)
