...
SNP_matrix <- read_tsv("variant_positions_combined.tsv", col_names = c("CHROM", "POS")) %>%
        left_join(
                read_table("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/PK_consensus_no_MOI.vcf", skip = 87) %>%
...
SNP_matrix %>% 
    ...
    filter((Mn > nrow(metadata %>% filter(Cluster == "Mn"))*0.5 & is.na(Mf) & is.na(Peninsular)) |
    (Mf > nrow(metadata %>% filter(Cluster == "Mf"))*0.5 & is.na(Mn) & is.na(Peninsular)) |
    (Peninsular > nrow(metadata %>% filter(Cluster == "Peninsular"))*0.5 & is.na(Mf) & is.na(Mn))) %>%
    select(CHROM, POS) %>%
    write_tsv("SNPs_segregated_all.txt")
