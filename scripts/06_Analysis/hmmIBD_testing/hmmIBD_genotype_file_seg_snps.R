...
#Prop
read_tsv("variant_positions_segregated_prop.tsv", col_names = c("CHROM", "POS")) %>%
        left_join(
        ...
        select(-c(3:9)) %>%
        arrange(CHROM, POS) %>%
        write_tsv("hmmIBD_subset_prop.tsv") 

#All
read_tsv("variant_positions_segregated_all.tsv", col_names = c("CHROM", "POS")) %>%
        left_join(
                read_table("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/PK_consensus_no_MOI.vcf", skip = 87) %>%
                rename("CHROM" = `#CHROM`) # this first section selects only variants that passed the PLINK filters we applied
        ) %>%
        ...
        select(-c(3:9)) %>%
        arrange(CHROM, POS) %>%
        write_tsv("hmmIBD_subset_all.tsv") 