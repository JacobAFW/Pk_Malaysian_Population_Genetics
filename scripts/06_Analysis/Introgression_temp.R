...
# filter out windows that have SICAvar of Kir genes present
...
######################################################################################################################## WIP
# Filter windows that appear in both Mn & Mf clusters - suggests hypervariability?
...
PKA1H1_windows %>% 
    filter(WINDOW %in% hypervariable_filter$WINDOW) %>% 
...
######################## Mf samples
introgressed_windows_filter <- introgressed_windows %>% # filter for moderate frequency windows
    filter(Cluster == "Mf" & Mf == 0 & Mn == 1) %>%
    dplyr::select(WINDOW) %>% 
    group_by(WINDOW) %>% 
    summarise(n = n()/81*100) %>%
    filter(n > 5) # filter for windows that show up in at least 5% of samples
...
introgressed_windows_filter <- introgressed_windows %>% # filter for moderate frequency windows
    ...
    dplyr::select(WINDOW) %>% 
    group_by(WINDOW) %>% 
    summarise(n = n()/38*100) %>%
    filter(n > 5) # filter for windows that show up in at least 5% of samples - Mn specific
...
# PKA1H1_windows %>% dplyr::select(CHROM, WINDOW) %>% unique() %>% group_by(CHROM) %>% summarise(n()) - reference point - total windows per chromosome 
...
distance_plot <- genetic_distance_window_plot(introgression_table_window, 906)
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/Introgression/window_plot_906_Mf.png", dpi = 300, distance_plot)
...
# Mn
distance_plot <- genetic_distance_window_plot(introgression_table_window, 1118)
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/Introgression/window_plot_1118_Mn.png", dpi = 300, distance_plot)

distance_plot <- genetic_distance_window_plot(introgression_table_window, 795)
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/Introgression/window_plot_795_Mn.png", dpi = 300, distance_plot)

distance_plot <- genetic_distance_window_plot(introgression_table_window, 124)
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/Introgression/window_plot_124_Mn.png", dpi = 300, distance_plot)
...
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/Introgression/tree_plots/NJT_tree_window_124_Mn_unrooted.png", dpi = 300, height = 15, width = 15, limitsize = FALSE, NJT_tree_plot)
...
ggsave("/g/data/pq84/malaria/Pk_Malaysian_Population_Genetics/outputs/05_Analyses/Introgression/tree_plots/NJT_tree_window_124_Mn_unrooted_labelled.png", dpi = 300, height = 20, width = 20, limitsize = FALSE, NJT_tree_plot)
